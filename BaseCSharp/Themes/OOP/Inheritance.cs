...
public class Inheritance
{
    ...
    internal class Employee : Person // наследуется от Person и имеет весь его функционал кроме конструкторов с параметрами!
    {
        ...
        {
            // сначала вызовитсЯ по цепочке base потом все потомки до Object и обратно и только потом этот конструктор
            _age = age;
        }
    ...
    public void Run()
    {
        ...
        //employee3.Log2() - нет такого метода тк видны только методы на уровне типа а у object такого метода нет
        
        employee2.Log2(); // такой метод есть тк тип Employee
        // те базовый тип не видит функционал потомков, а потомки видят функционал базовых классов
